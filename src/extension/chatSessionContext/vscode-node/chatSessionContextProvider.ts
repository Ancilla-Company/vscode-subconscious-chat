...
class ContextResolver implements Copilot.ContextResolver<Copilot.SupportedContextItem> {
	...
	private async generateSummary(conversation: Conversation): Promise<string | undefined> {
		try {
			...
			const models = await vscode.lm.selectChatModels({ family: 'gpt-4o-mini', vendor: 'subconscious' });
			if (models.length === 0) {
				// Fallback to any available model
				const allModels = await vscode.lm.selectChatModels({ vendor: 'subconscious' });
